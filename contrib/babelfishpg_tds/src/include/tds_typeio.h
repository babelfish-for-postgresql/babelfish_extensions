...
ColMetaEntry2
{
	...
	/*
	 * collationInfo(32 bits): LCID/CodePage (20 bits) + collationFlags(8
	 * bits) + version (4 bits)
	 */
...
typedef struct TdsRelationMetaData
{
	 ...
	 *
	 * partName[0] - relation name partName[1] - schema name partName[2] -
	 * database name partName[3] - object name
	 */
	char	   *partName[4];
...
typedef struct ParameterTokenData
{
	...
	/*
	 * maxlen and len fields are 4 bytes for some datatypes(text, ntext) while
	 * 2 bytes for (nvarchar, others?) and 1 byte for others.
	 */
	uint32_t	maxLen;
	uint32_t	len;
	bool		isNull;
	...
	/*
	 * dataOffset points to the offset in the request message from where the
	 * data bytes actually start. Using, dataOffset + len we can fetch the
	 * entire data from the request message, when we want to use it.
	 */
	int			dataOffset;
	 ...
	 * function.  For IN parameters, we use the receiver functions to convert
	 * the parameter from TDS wire format to Datum.  For OUT parameters, we
	 * use the sender functions to convert the Datums to TDS wire format and
	 * include them in the return value tokens.
	 */

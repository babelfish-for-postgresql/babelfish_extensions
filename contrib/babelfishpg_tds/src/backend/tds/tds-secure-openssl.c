 ...
 *
 * IDENTIFICATION
 *	  contrib/babelfishpg_tds/src/backend/tds/tds-be-secure-openssl.c
 *
 *-------------------------------------------------------------------------
 */
...
int
Tds_be_tls_open_server(Port *port)
{
	int			r;
	int			err;
	int			waitfor;
...
ssize_t
Tds_be_tls_read(Port *port, void *ptr, size_t len, int *waitfor)
{
	ssize_t		n;
	int			err;
	unsigned long ecode;
...
ssize_t
Tds_be_tls_write(Port *port, void *ptr, size_t len, int *waitfor)
{
	ssize_t		n;
	int			err;
	unsigned long ecode;
...
my_SSL_set_fd(Port *port, int fd)
{
	...
	/* Tds specific SSL function register */
	bio_method = TdsBioSecureSocket(my_bio_methods);
	my_bio_methods = bio_method;

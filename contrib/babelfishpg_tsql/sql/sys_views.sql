...
CREATE OR REPLACE FUNCTION sys.dm_os_sys_info()
RETURNS TABLE (
    cpu_ticks bigint,
    ms_ticks bigint,
    cpu_count integer,
    ...
    sqlserver_start_time sys.DATETIME,
    affinity_type integer,
    affinity_type_desc sys.NVARCHAR,
    process_kernel_time_ms bigint,
    process_user_time_ms bigint,
    time_source integer,
    time_source_desc sys.NVARCHAR,
    virtual_machine_type integer,
    virtual_machine_type_desc sys.NVARCHAR,
    softnuma_configuration integer,
    softnuma_configuration_desc sys.NVARCHAR,
    process_physical_affinity sys.NVARCHAR,
    sql_memory_model integer,
    sql_memory_model_desc sys.NVARCHAR,
    socket_count integer,
    cores_per_socket integer,
    numa_node_count integer,
    container_type integer,
    container_type_desc sys.NVARCHAR
) 
LANGUAGE plpgsql
AS $$
BEGIN
    RETURN QUERY 
    SELECT 
        0::bigint AS cpu_ticks,
        ROUND(EXTRACT(EPOCH FROM NOW()) * 1000.0::numeric)::bigint AS ms_ticks,
        0::integer AS cpu_count,
        0::integer AS hyperthread_ratio,
        0::bigint AS physical_memory_kb,
        0::bigint AS virtual_memory_kb,
        0::bigint AS committed_kb,
        0::bigint AS committed_target_kb,
        0::bigint AS visible_target_kb,
        0::integer AS stack_size_in_bytes,
        0::bigint AS os_quantum,
        0::integer AS os_error_mode,
        0::integer AS os_priority_class,
        0::integer AS max_workers_count,
        0::integer AS scheduler_count,
        0::integer AS scheduler_total_count,
        0::integer AS deadlock_monitor_serial_number,
        ROUND(EXTRACT(EPOCH FROM pg_postmaster_start_time()) * 1000.0::numeric)::bigint AS sqlserver_start_time_ms_ticks,
        pg_postmaster_start_time()::sys.DATETIME AS sqlserver_start_time,
        0::integer AS affinity_type,
        NULL::sys.NVARCHAR AS affinity_type_desc,
        0::bigint AS process_kernel_time_ms,
        0::bigint AS process_user_time_ms,
        0::integer AS time_source,
        NULL::sys.NVARCHAR AS time_source_desc,
        0::integer AS virtual_machine_type,
        'NONE'::sys.NVARCHAR AS virtual_machine_type_desc,
        0::integer AS softnuma_configuration,
        'OFF'::sys.NVARCHAR AS softnuma_configuration_desc,
        NULL::sys.NVARCHAR AS process_physical_affinity,
        0::integer AS sql_memory_model,
        NULL::sys.NVARCHAR AS sql_memory_model_desc,
        0::integer AS socket_count,
        0::integer AS cores_per_socket,
        0::integer AS numa_node_count,
        0::integer AS container_type,
        NULL::sys.NVARCHAR AS container_type_desc;
END;
$$;

GRANT EXECUTE ON FUNCTION sys.dm_os_sys_info() TO PUBLIC;

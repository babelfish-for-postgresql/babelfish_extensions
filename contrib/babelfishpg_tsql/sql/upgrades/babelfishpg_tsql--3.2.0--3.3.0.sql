...
=======
>>>>>>> b64d9d67 (move sql script to before reset to fix mvu issue)
...
-- Reset search_path to not affect any subsequent scripts
SELECT set_config('search_path', trim(leading 'sys, ' from current_setting('search_path')), false);
>>>>>>> aed2c7f6 (Supporting connect timeout server option (#1576))
=======
...
<<<<<<< HEAD
>>>>>>> 979d3630 (add sql script for upgrade test)
=======
/*
 * tsql full-text search configurations for Babelfish
 */
...
>>>>>>> 0ff0bad1 (move sql script to before reset to fix mvu issue)

...
-- information_schema_tsql.column_privileges source

CREATE OR REPLACE VIEW information_schema_tsql.column_privileges
AS SELECT u_grantor.rolname::sys.nvarchar(128) AS grantor,
...
WHERE x.relnamespace = nc.oid AND x.grantee = grantee.oid 
AND x.grantor = u_grantor.oid AND (x.prtype = ANY (ARRAY['INSERT'::text, 'SELECT'::text, 'UPDATE'::text, 'REFERENCES'::text])) 

...
select 
  ...
  , CAST((ss.x).n AS INT) AS parameter_id
  -- 'system_type_id' is specified as type INT here, and not TINYINT per SQL Server documentation.
  -- This is because the IDs of generated SQL Server system type values generated by
  -- Babelfish installation will exceed the size of TINYINT
  ...
  , CAST(
    CASE
      WHEN ss.prokind = 'f' AND ss.proargnames[(ss.x).n] IS NULL THEN 1 -- Output of a scalar function
      WHEN ss.proargmodes[(ss.x).n] = 'i'::"char" THEN 0
      WHEN ss.proargmodes[(ss.x).n] = 'o'::"char" THEN 1
      WHEN ss.proargmodes[(ss.x).n] = 'b'::"char" THEN 1
      WHEN ss.proargmodes[(ss.x).n] = 'v'::"char" THEN 0
      WHEN ss.proargmodes[(ss.x).n] = 't'::"char" THEN 1
      ELSE 0
    END 
  ...
  , CAST(NULL AS sys.bit) AS default_value

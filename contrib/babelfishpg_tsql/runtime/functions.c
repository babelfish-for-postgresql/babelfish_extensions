...
parsename(PG_FUNCTION_ARGS)
{
	...
	for (int i = 0; i < len; i++)
	{
		...
		int part_len = u_strlen(&object_parts[current_part * 129]);
		if (state == 0) // Outside quotes and brackets
		{
			...
			else
			{
				if (part_len < 128)
				{
					object_parts[current_part * 129 + part_len] = c;
					object_parts[current_part * 129 + part_len + 1] = '\0';
				}
				else
				{
					pfree(object_name_str);
	...
	if (u_strlen(&object_parts[(current_part - object_piece + 1) * 129]) > 0)
	{
		// Convert the result from Unicode to UTF-8
		char utf8_result[256] = {0};
		text *result;
		u_strToUTF8(utf8_result, sizeof(utf8_result), NULL, &object_parts[(current_part - object_piece + 1) * 129], -1, &errorCode);
		if (U_FAILURE(errorCode))
		{
			pfree(object_name_str);
			PG_RETURN_NULL();

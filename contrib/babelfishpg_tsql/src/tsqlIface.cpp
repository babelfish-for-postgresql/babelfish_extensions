...
public:
	...
	/*
	 * Following function will help in rewriting query with syntax as follows:
	...
	void exitSTRING_AGG(TSqlParser::STRING_AGGContext *ctx) override
	{
		 ...
		 * hence we cannot rewrite the STRING_AGG to sys.STRING_AGG during creation of this objects, as it will then no longer
		 * follow pltsql syntax, hence skipped the rewriting of STRING_AGG query for Function/Procedure/Trigger. 
		 ...
		 */
		if (ctx->STRING_AGG() && !(in_create_or_alter_function || in_create_or_alter_procedure || in_create_or_alter_trigger))
		{

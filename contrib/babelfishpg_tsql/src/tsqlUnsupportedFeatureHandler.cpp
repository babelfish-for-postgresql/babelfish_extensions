...
declare_escape_hatch(escape_hatch_inline_function);
...
antlrcpp::Any TsqlUnsupportedFeatureHandlerImpl::visitCreate_or_alter_function(TSqlParser::Create_or_alter_functionContext *ctx)
{
	...
	for (auto option : options)
	{
		...
		else if (option->execute_as_clause())
		{
			...
			handle(INSTR_UNSUPPORTED_TSQL_CREATE_FUNCTION_INLINE_OPTION, "INLINE", &st_escape_hatch_inline_function, getLineAndPos(option->inline_clause()));
	}

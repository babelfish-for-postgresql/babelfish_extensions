...
create_bbf_authid_user_ext(CreateRoleStmt *stmt, bool has_schema, bool has_login, bool from_windows)
{
	...
	if (has_login)
	{
		...
		PG_END_TRY();
		grant_revoke_role_to_login(login_name_str, get_guest_role_name(db_name), BABELFISH_ROLE_ADMIN, false); /* revoke even membership granted by bbf_role_admin */
		pfree(db_name);
	}
	...
	while (HeapTupleIsValid(tuple))
	{
		...
		if (!is_null)
		{
			...
			if (strlen(login) > 0)
			{
				...
				grant_revoke_role_to_login(login, get_guest_role_name(db_name), BABELFISH_ROLE_ADMIN, false); /* revoke even membership granted by bbf_role_admin */
				pfree(db_name);
			}
		}
		tuple = heap_getnext(scan, ForwardScanDirection);
...
alter_bbf_authid_user_ext(AlterRoleStmt *stmt)
{
	...
	if (login_name_str)
	{
		...
		if (old_login_name && strlen(old_login_name) > 0)
		{
			...
			grant_revoke_role_to_login(old_login_name, stmt->role->rolename, BABELFISH_ROLE_ADMIN, false);
		...
		PG_END_TRY();
		grant_revoke_role_to_login(login_name_str, get_guest_role_name(get_cur_db_name()), BABELFISH_ROLE_ADMIN, false); /* revoke even membership granted by bbf_role_admin */
	}

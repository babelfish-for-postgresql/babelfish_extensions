...
sp_describe_undeclared_parameters_internal(PG_FUNCTION_ARGS)
{
	...
	if (call_cntr < max_calls)
	{
		...
		"WHEN \'xml\' THEN 241 "
		"ELSE CASE "
			"WHEN T2.name IS NOT NULL OR t.typbasetype = 0 THEN "
		...
		"WHEN \'decimal\' THEN \'decimal(\' + CAST( CASE "
				"WHEN a.atttypmod != -1 THEN "
					"sys.tsql_type_precision_helper(type, a.atttypmod) "
				"ELSE "
					"sys.tsql_type_precision_helper(type, t.typtypmod) "
			"END  AS sys.VARCHAR(10) ) + \',\' + CAST( CASE "
				"WHEN a.atttypmod != -1 THEN "
					"sys.tsql_type_scale_helper(type, a.atttypmod, false) "
				"ELSE "
					"sys.tsql_type_scale_helper(type, t.typtypmod, false) "
		...
		"WHEN \'char\' THEN \'char(\' + CAST( CASE "
				"WHEN a.atttypmod != -1 THEN "
					"sys.tsql_type_max_length_helper(type, a.attlen, a.atttypmod) "
				"ELSE "

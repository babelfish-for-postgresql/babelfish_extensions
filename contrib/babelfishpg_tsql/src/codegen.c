...
cleanup_exception_context(PLtsql_stmt *src, PLtsql_stmt *dst, CodegenContext *codegen_ctx)
{
    ...
    DynaVec *src_trycatch_infos, *dst_trycatch_infos;
    size_t src_depth, dst_depth;
...
static bool stmt_goto_act(Walker_context *ctx, PLtsql_stmt_goto *stmt)
{
    ...
    /* 
     * handle dangling exception contexs if any
     * if target label is outside of current try-catch block"
     * goto inner try-catch block was blocked in analyzer
     */
...
void resolve_labels(Walker_context *ctx)
{
    ...
    /*  Fill missing goto targets
     *  targe_pc could be only filled partially during tree traversal
     *  when label is defined after GOTO
     */
    for (i = 0; i < size; i++)
    {

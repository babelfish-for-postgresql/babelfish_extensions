...
static inline bool is_integer_type(Oid type) {
	return type == INT2OID || type == INT4OID || type == INT8OID;
}

static bool lower_precision_than(Oid int_type1, Oid int_type2) {
...
static Node* optimize_explicit_cast(ParseState *pstate, Node *node)
{
		...
		((FuncExpr *) node)->funcformat == COERCE_EXPLICIT_CAST)
	{
		 ...
		 */
		while (((FuncExpr *) node)->funcformat == COERCE_EXPLICIT_CAST) {
			FuncExpr *f = (FuncExpr *) node;

			if (!is_integer_type(f->funcresulttype)) break;

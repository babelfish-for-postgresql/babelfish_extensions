				...
				for (pp = numposargs; pp < pronargs; pp++)
				{
					...
					if (default_positions_available)
					{
						...
						if (!has_default)
							ereport(ERROR,
									(errcode(ERRCODE_UNDEFINED_FUNCTION),
									 errmsg("%s %s expects parameter \"%s\", which was not supplied.", obj_type, NameListToString(names), bbf_get_original_parameter_name(p_argnames[pp], candidates->oid))),
									parser_errposition(pstate, location));
					}
				...
				if (proc_form->prokind == PROKIND_PROCEDURE)
					ereport(ERROR,
						...
						errmsg("Procedure or function \'%s\' expects parameter \'%s\', which was not supplied.",
							NameStr(proc_form->proname), bbf_get_original_parameter_name(arg_names[i], proc_form->oid))));
			}
			++i;
		}
		heap_freetuple(bbffunctuple);

...
/*
 * partition_function_exists_with_id
 * 		Helper function to generate new ids for partition function,
 * 		it check if provided id is already in use.
 */
static bool
partition_function_exists_with_id(int32 id)
{
	Relation	rel;
	HeapTuple	tuple;
	SysScanDesc	scan;
	...
	systable_endscan(scan);
	table_close(rel, AccessShareLock);
	return partition_function_exists;
}

/*
 * get_available_partition_function_id
 * 		Generates new ids for partition scheme.
...
get_available_partition_function_id(void)
{
	...
	} while (partition_function_exists_with_id(id));
...
/*
 * partition_scheme_exists_with_id
 * 		Helper function to generate new ids for partition scheme, 
 * 		it check if provided id is already in use.
 */
static bool
partition_scheme_exists_with_id(int32 id)
{
	Relation	rel;
	HeapTuple	tuple;
	SysScanDesc	scan;
	...
	systable_endscan(scan);
	table_close(rel, AccessShareLock);
	return partition_scheme_exists;
}
...
get_available_partition_scheme_id(void)
{
	...
	} while (partition_scheme_exists_with_id(id));
 ...
 * 		checks if partition function is used by any partition scheme
 */
bool is_partition_function_used(int32 partition_function_id)
{
	Relation	rel;
	HeapTuple	tuple;
	SysScanDesc	scan;

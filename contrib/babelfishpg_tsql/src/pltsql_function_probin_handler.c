...
bool
pltsql_function_as_checker(const char *lang, List *as, char **prosrc_str_p, char **probin_str_p)
{
	/*
	 * This hook checks if it's pltsql language and if we have two AS clauses
	 * (probin+prosrc). We'll populate the probin and prosrc strings with the
	 * AS clauses here and later we'll skip the generation of new probin
	 * string in write_stored_proc_probin_hook function.
	 */
...
void
pltsql_function_probin_writer(CreateFunctionStmt *stmt, Oid languageOid, char **probin_str_p)
{
	char	   *langname;
	int			probin_len;
	Jsonb	   *jb;
							  ...
							  Oid funcid)
{
	...
	else
	{
		...
		if (argmodes)
		{
			...
			{
				/*
				 * We only stored typmod of parameter with mode INPUT/'i',
				 * OUTPUT/'o' and INOUT/'b' Refer to PROARGMODE_IN in
				 * pg_proc.h
				 */
		...
		/*
		 * No argmodes. Meaning every argument is IN arg. We can simply use
		 * numargs
		 */
		else
		{
		...
		/*
		 * BABEL-2392 There can be less arguments in the EXEC PROC statement
		 * compared to the full list of arguments desired by the procedure.
		 * Also the named arguments in the EXEC PROC statement can appear in
		 * any order. So we have to get the full typmod_array using the number
		 * of IN or OUT parameters instead of fargs.length. Also we need to
		 * match the argument name and then invoke adjustTypmod on the
		...
		foreach(current_fargs, fargs)
		{
			...
			if (IsA(node, NamedArgExpr))
			{
				...
				/*
				 * BABEL-2392 Arguments in EXEC PROC statement can be in any
				 * order such as exec mysp @arg2 = 1, @arg1 = 'abc' Look for
				 * the matching argument name then invoke adjustTypmod on the
				 * corresponding typmod_array element
				 */
				...
				/*
				 * Name unmatched, this should not happen since we've done
				 * checks during parse analysis. But just in case
				 */
				else
				{
...
void
probin_read_args_typmods(HeapTuple procTup, int nargs, Oid *argtypes, int **typmods)
{
	bool		isnull;
	char	   *probin_c = NULL;
...
int
probin_read_ret_typmod(Oid funcid, int nargs, Oid declared_oid)
{
	int		   *typmod_array = NULL;
	int			arr_len = nargs + 1;
	int			ret_typmod = -1;
...
static char *
catalog_read_probin(Oid funcid)
{
	bool		isnull;
	HeapTuple	ftup;
	Datum		tmp;
...
static void
buildTypmodArray(CreateFunctionStmt *stmt, int **typmod_array_p, int *array_len_p)
{
	A_Const    *ptr;
	List	   *arg_typmod;
	ListCell   *x;
...
int
adjustTypmod(Oid oid, int typmod)
{
	Type		baseType;
	char	   *typname;

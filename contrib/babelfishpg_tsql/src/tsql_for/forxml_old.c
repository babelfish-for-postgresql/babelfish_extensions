						...
						const char* element_name, bool binary_base64)
{
	...
	if (binary_base64)
	{
		 ...
		 * Refer to how BYTEA type is handed in map_sql_value_to_xml_value().
												...
												const char *root_name)
{
	...
	switch (mode)
	{
		...
		case TSQL_FORXML_AUTO:
			 ...
			 * value being processed, but relation id/name is not provided by SPI. We need to make
			 * relation id available in SPI_tuptable->tupdesc in order to support AUTO mode.
			 */
			ereport(ERROR,
							(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),

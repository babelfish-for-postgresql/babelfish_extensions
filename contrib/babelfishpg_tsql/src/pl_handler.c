...
pltsql_truncate_identifier(char *ident, int len, bool warn)
{
	...
	if (warn)
		original = pnstrdup(ident, len);

	(*pltsql_protocol_plugin_ptr)->truncate_identifier(ident, len,
			tsql_is_database_or_server_collation_CI());
...
pltsql_cstr_to_name(char *s, int len)
{
	...
	if (len >= NAMEDATALEN)
	{
		if (sql_dialect == SQL_DIALECT_TSQL)
		{
			char	   *tmp = pnstrdup(s, len);

			(*pltsql_protocol_plugin_ptr)->truncate_identifier(tmp, len,
					tsql_is_database_or_server_collation_CI());
			s = tmp;
			len = strlen(s);
		}

...
// void debug_print_hash_table(HTAB *hash);
...
pltsql_post_parse_analyze(ParseState *pstate, Query *query, JumbleState *jstate)
{
	...
	if (Debug_print_parse)
		elog_node_display(LOG, "Query tree before check_json_auto_walker", query,
						  Debug_pretty_print);
 ...
 * Input:
 * - targetList:
 * The subquery->targetList where the subquery originates from the original
 * select_from_clause. These should be the keys at the outermost json struct.
 *  
 * - tableInfoArr:
 * - numTables
 * 

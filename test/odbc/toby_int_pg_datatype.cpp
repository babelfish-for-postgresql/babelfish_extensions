...
TEST_F(PSQL_DataTypes, Datatypes_bigint) {
  ...
  const string TABLE_NAME = "int_table";
  const string COL1_NAME = "pk"; // primary key
  const string COL2_NAME = "data";
  const string DATATYPE_NAME = "sys.integer";
  ...
  const string NAME_EXPECTED = "int4";   // int is also fine
  ...
  vector<pair<string, string>> INT_TABLE_COLUMNS = {
  ...
  odbcHandler.ConnectAndExecQuery(CreateTableStatement(TABLE_NAME, INT_TABLE_COLUMNS));
  ...
  for (int i = 0; i < NUM_ROWS; ++i) {
    ...
    if (inserted_values[i] != "NULL")
    {
      ...
      ASSERT_EQ(data, strtol(inserted_values[i].c_str(), NULL, 10));
    } 
    else 

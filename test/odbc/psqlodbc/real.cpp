...
vector<float> getExpectedFloatResults(vector<string> data) {
...
TEST_F(PSQL_DataTypes_Real, Insertion_Success) {
  ...
  const vector<float> expected = getExpectedFloatResults(VALID_INSERTED_VALUES);

  const vector<long> expectedLen(expected.size(), FLOAT_BYTES_EXPECTED);

  createTable(ServerType::PSQL, TABLE_NAME, TABLE_COLUMNS);
  testInsertionSuccess(ServerType::PSQL, TABLE_NAME, COL1_NAME, SQL_C_FLOAT, data, bufferLen, VALID_INSERTED_VALUES, 
...
TEST_F(PSQL_DataTypes_Real, Update_Success) {
  ...
  const vector<float> EXPECTED_DATA_INSERTED = getExpectedFloatResults(DATA_INSERTED);
  ...
  const vector<float> DATA_UPDATED_EXPECTED = getExpectedFloatResults(DATA_UPDATED_VALUES);

  const vector<long> expectedLen(DATA_UPDATED_EXPECTED.size(), FLOAT_BYTES_EXPECTED);

  createTable(ServerType::PSQL, TABLE_NAME, TABLE_COLUMNS);
  testInsertionSuccess(ServerType::PSQL, TABLE_NAME, COL1_NAME, SQL_C_FLOAT, data, bufferLen, DATA_INSERTED, 
    EXPECTED_DATA_INSERTED, expectedInsertLen);
  testUpdateSuccess(ServerType::PSQL, TABLE_NAME, COL1_NAME, COL2_NAME, SQL_C_FLOAT, data, bufferLen, DATA_UPDATED_VALUES, 
...
TEST_F(PSQL_DataTypes_Real, Update_Fail) {
  ...
  testInsertionSuccess(ServerType::PSQL, TABLE_NAME, COL1_NAME, SQL_C_FLOAT, data, bufferLen, DATA_INSERTED, 
    EXPECTED_DATA_INSERTED, expectedInsertLen);
  testUpdateFail(ServerType::PSQL, TABLE_NAME, COL1_NAME, COL2_NAME, SQL_C_FLOAT, data, bufferLen, EXPECTED_DATA_INSERTED, 
...
TEST_F(PSQL_DataTypes_Real, Arithmetic_Functions) {
  ...
  createTable(ServerType::PSQL, TABLE_NAME, TABLE_COLUMNS);
  insertValuesInTable(ServerType::PSQL, TABLE_NAME, insertString, NUM_OF_DATA);
  testComparisonFunctions(ServerType::PSQL, TABLE_NAME, SQL_C_FLOAT, col_results, bufferLen, OPERATIONS_QUERY, expected_results, expectedLen);
  dropObject(ServerType::PSQL, "TABLE", TABLE_NAME);
}
...
TEST_F(PSQL_DataTypes_Real, View_Creation) {
  ...
  const vector<float> EXPECTED_DATA = getExpectedFloatResults(VALID_INSERTED_VALUES);

  const vector<long> expectedLen(EXPECTED_DATA.size(), FLOAT_BYTES_EXPECTED);

  createTable(ServerType::PSQL, TABLE_NAME, TABLE_COLUMNS);
  testInsertionSuccess(ServerType::PSQL, TABLE_NAME, COL1_NAME, SQL_C_FLOAT, data, bufferLen, VALID_INSERTED_VALUES, EXPECTED_DATA, expectedLen);

  createView(ServerType::PSQL, VIEW_NAME, VIEW_QUERY);
  verifyValuesInObject(ServerType::PSQL, VIEW_NAME, COL1_NAME, SQL_C_FLOAT, data, bufferLen, VALID_INSERTED_VALUES, EXPECTED_DATA, expectedLen);

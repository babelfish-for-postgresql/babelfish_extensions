...
size_t GetExpectedSize(std::string expected_hex, size_t table_size = -1) {
...
TEST_F(PSQL_DataTypes_VarBinary, Insertion_Success) {
  ...
  for (int i = 0; i < NUM_OF_INSERTS; i++) {
    ...
    if (VALID_INSERTED_VALUES[i] != "NULL") {
      ...
      size_t expected_size = GetExpectedSize(expected_hex);

      // NOTE - BBF does not prepend `0x` to return data, whilst PG does
      // +2 for the prepended `0x`
      ASSERT_EQ(data_len, expected_size + 2);
      ASSERT_TRUE(BinaryCompare(data, expected_hex, expected_size));
    }
    else {
...
TEST_F(PSQL_DataTypes_VarBinary, Comparison_Operators) {
  ...
  for (int i = 0; i < NUM_OF_DATA; i++) {
    odbcHandler.CloseStmt();
    odbcHandler.ExecQuery(SelectStatement(TABLE_NAME, OPERATIONS_QUERY, vector<string>{COL1_NAME}, COL1_NAME + " operator(sys.=) cast(" + INSERTED_PK[i] + "as sys.varbinary)"));
    ASSERT_NO_FATAL_FAILURE(odbcHandler.BindColumns(BIND_COLUMNS));
...
TEST_F(PSQL_DataTypes_VarBinary, Table_Unique_Constraints) {
  const vector<pair<string, string>> TABLE_COLUMNS = {
    {COL1_NAME, "INT PRIMARY KEY"},
...
TEST_F(PSQL_DataTypes_VarBinary, Table_Composite_Keys) {
  const vector<pair<string, string>> TABLE_COLUMNS = {
    {COL1_NAME, "INT"},
    {COL2_NAME, DATATYPE_NAME}
  };
...
TEST_F(PSQL_DataTypes_VarBinary, 8000_Byte_Insertion_Success) {
  ...
  const size_t DATA_LENGTH = DATA_INSERTED.length();
  ...
  const vector<tuple<int, int, SQLPOINTER, int, SQLLEN *>> BIND_COLUMNS = {
    {1, SQL_C_LONG, &pk, 0, &pk_len},
    {2, SQL_C_CHAR, &data, MAX_BUFFER_SIZE, &data_len}
  };
  ...
  // NOTE - BBF does not prepend `0x` to return data, whilst PG does
  // +2 for the prepended `0x`
  ASSERT_EQ(data_len, DATA_LENGTH + 2);
  ASSERT_TRUE(BinaryCompare(data, DATA_INSERTED, DATA_LENGTH));

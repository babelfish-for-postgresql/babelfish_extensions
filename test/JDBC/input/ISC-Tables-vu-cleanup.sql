DROP TABLE isc_tables_sc1.t2
DROP TABLE [ISC_TABLES_TABLE_SCHEMA].[ISC_TABLES_TABLE]
DROP TABLE [ISC_TABLES SCHEMA . WITH .. DOTS].[ISC_TABLES TABLE . WITH .. DOTS]
GO

DROP TYPE isc_tables_sc1.isc_table_type2
GO

DROP SCHEMA isc_tables_sc1
DROP SCHEMA [ISC_TABLES_TABLE_SCHEMA]
DROP SCHEMA [ISC_TABLES SCHEMA . WITH .. DOTS]
GO

DROP VIEW isc_tables_vu_prepare_v1
GO

DROP FUNCTION isc_tables_vu_prepare_f2
DROP FUNCTION isc_tables_vu_prepare_f1
GO

DROP PROCEDURE isc_tables_vu_prepare_p1
GO

DROP TABLE isc_tables_vu_prepare_t1
GO

DROP TYPE isc_table_type1
GO

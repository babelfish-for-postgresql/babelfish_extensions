...
CREATE TABLE babel_5512_t5 ( ID INT IDENTITY(1,1), Amount Int, a_tiny tinyint, b_smallint smallint, c_big bigint, d_fl float, e_real real, f_mon money, g_smallmoney smallmoney);
GO

SELECT SUM(Amount) * 1.1 AS TotalWithMarkup, sum(a_tiny) * 1.1 as mul1, sum(b_smallint) * 1.1 as mul2, sum(c_big) * 1.1 as mul3, sum(d_fl) * 1.1 as mul4, sum(e_real) * 1.1 as mul5, sum(f_mon) * 1.1 as mul6, sum(g_smallmoney) * 1.1 as mul7 INTO babel_5512_t6 FROM babel_5512_t5;
GO

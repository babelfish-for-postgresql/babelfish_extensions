...
CREATE FUNCTION str_vu_prepare_f1
(
    @x   float,
    @len int,
    @dec int
...
AS
BEGIN
    RETURN STR(@x, @len, @dec);
END
GO

CREATE FUNCTION str_vu_prepare_f2
(
    @len int,
    @dec int
)
...
(
    SELECT
        ...
        v.val,
        STR(v.val, @len, @dec) AS formatted
    FROM (VALUES
          (CAST(  1.9999 AS float)),  -- carry --> ' 2.000'
          (CAST(  3.9999 AS float)),  -- carry --> ' 4.000'
          (CAST( -999.9  AS float)),  -- dec=0 negatif --> ' -1000'
          (CAST(  0.9995 AS float)),  -- boundary --> ' 1.000'
          (CAST(123.355  AS float)),  -- typical rounding
          (CAST(1234.5678 AS float))  -- length/decimal squeeze
    ) AS v(val)
);
GO
...
CREATE VIEW str_vu_prepare_v20 AS
(
    SELECT 
        str_vu_prepare_f1( 1.9999 , 6 , 3) AS udf_cr1,         -- ' 2.000'
        str_vu_prepare_f1( 3.9999 , 6 , 3) AS udf_cr2,         -- ' 4.000'
        str_vu_prepare_f1(-999.9  , 6 , 0) AS udf_neg_round0,  -- ' -1000'
        str_vu_prepare_f1(123.355 ,10 , 2) AS udf_round_basic  -- '    123.36'
);
GO
...
CREATE PROCEDURE str_vu_prepare_p20
AS
BEGIN
    SET NOCOUNT ON;
    SELECT * FROM dbo.str_vu_prepare_f2(6,3);  -- carry/rounding
END
GO

CREATE PROCEDURE str_vu_prepare_p21
AS
BEGIN
    SET NOCOUNT ON;

    SELECT 
        '[' + str_vu_prepare_f1(0.123456789012345678, 20, 20) + ']' AS pad1,
        '[' + str_vu_prepare_f1(0.123456789012345678, 18, 20) + ']' AS pad2,
        str_vu_prepare_f1(1234.5678, 8, 4)   AS squeeze1,    -- '1234.568'
        str_vu_prepare_f1(-999.9,     6, 0)  AS neg_round0;  -- ' -1000'
END
GO

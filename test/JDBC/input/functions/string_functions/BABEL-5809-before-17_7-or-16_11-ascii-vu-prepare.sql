...
CREATE TABLE babel_5809_t3_hex (
    ...
    ExpectedValue INT,
    ActualValue_Binary INT,
    ActualValue_VarBinary INT,
    TestResult_Binary VARCHAR(4),
    TestResult_VarBinary VARCHAR(4),
    Description VARCHAR(100)
);
GO
...
SET ActualValue_Binary = ASCII(BinaryCol),
    TestResult_Binary = CASE WHEN ASCII(BinaryCol) = ExpectedValue THEN 'Pass' ELSE 'Fail' END;
GO

UPDATE babel_5809_t3_hex
SET ActualValue_VarBinary = ASCII(VarbinaryCol),
    TestResult_VarBinary = CASE WHEN ASCII(VarbinaryCol) = ExpectedValue THEN 'Pass' ELSE 'Fail' END;
GO
...
SELECT 
    ...
    ActualValue_Binary,
    TestResult_Binary,
    ActualValue_VarBinary,
    TestResult_VarBinary,
    Description

CREATE TYPE dbo.babel_3409_concat_ws_imageUDT FROM image;
GO

CREATE TYPE dbo.babel_3409_concat_ws_varUDT FROM varchar(50);
GO
...
CREATE TABLE babel_3409_concat_ws_t1(
    id int,
    col_char char(20),
...
INSERT INTO babel_3409_concat_ws_t1 VALUES(
    1,
    N'abc',
    N'abc',
    N'abc',
...
CREATE TABLE babel_3409_concat_ws_t2(
    id int,
...
INSERT INTO babel_3409_concat_ws_t2 VALUES
(
    1,
    N'比尔·拉',
    N'比尔·拉',
...
AS
BEGIN
RETURN (SELECT TOP 1 CONCAT_WS('-', col_varchar, col_char, col_varbinary, col_text) FROM babel_3409_concat_ws_t1 ORDER BY id)
END
GO

CREATE FUNCTION babel_3409_concat_ws_itvf_func1()
RETURNS TABLE
AS
RETURN (SELECT TOP 1 CONCAT_WS('-', col_nvarchar, col_nchar, col_int, col_bigint, col_smallint, col_tinyint, col_numeric, col_float, col_real, col_bit) AS concatenated_column FROM babel_3409_concat_ws_t1 ORDER BY id)
GO

CREATE VIEW babel_3409_concat_ws_dep_view2 AS
    SELECT CONCAT_WS(N'-', col_nvarchar, col_nchar, col_varchar, col_decimal, col_smallmoney, col_smalldatetime) AS concatenated_column FROM babel_3409_concat_ws_t1
GO

CREATE PROCEDURE babel_3409_concat_ws_dep_proc2 AS
    SELECT CONCAT_WS(N'-', col_nvarchar, col_money, col_datetime, col_datetime2, col_binary, col_date, col_time, col_datetimeoffset) FROM babel_3409_concat_ws_t1 ORDER BY id
GO

CREATE FUNCTION babel_3409_concat_ws_dep_func2()
RETURNS NVARCHAR(50)
AS
BEGIN
RETURN (SELECT TOP 1 CONCAT_WS(N'-', col_nvarchar, col_varbinary, col_text, col_ntext) FROM babel_3409_concat_ws_t1 ORDER BY id)
END
GO

CREATE FUNCTION babel_3409_concat_ws_itvf_func2()
RETURNS TABLE
AS
RETURN (SELECT TOP 1 CONCAT_WS(N'-', col_nvarchar, col_varchar, col_char, col_nchar) AS concatenated_column FROM babel_3409_concat_ws_t1 ORDER BY id)
GO

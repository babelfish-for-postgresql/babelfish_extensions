...
WHILE @i <= 200
BEGIN
    DECLARE @basePhrase VARCHAR(100)
    ...
    SET @basePhrase = 'Project-' + RIGHT('000' + CAST(@i AS VARCHAR(3)), 3) + 
                      ': ' + @category + ' ' + @phase
    ...
    VALUES
    (
        @basePhrase + ' - This comprehensive technical documentation details the ' +
        'implementation specifics and architectural considerations for ' + @category + 
        ' systems during the ' + @phase + ' phase.',
        
        LEFT(@basePhrase + ' [Doc-Ref: ' + CAST(@i AS VARCHAR(3)) + ']', 50),
        
        @category + ' Initiative ' + CAST(@i AS VARCHAR(3)) + ': ' + @phase + 
        ' phase documentation',
        
        N'Tech-Doc-' + CAST(@i AS VARCHAR(3)) + N': ' + @basePhrase + 
        N' (系统文档)',
        
        'Document ID ' + CAST(@i AS VARCHAR(3)) + ': Extended technical specification for ' +
        @basePhrase + '. This document provides in-depth coverage of ' + @category + 
        ' implementation during ' + @phase + ' phase, including best practices, ' +
        'requirements, and technical guidelines.',
        
        N'Ref-' + RIGHT('000' + CAST(@i AS VARCHAR(3)), 3) + N': ' + 
        @category + N' ' + @phase + N' 技术规范'
    )

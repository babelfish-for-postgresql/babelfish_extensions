...
CREATE VIEW prefix_rewrite_prepare_v21 AS (SELECT sys.babelfish_fts_rewrite('"*"'));
GO


-- Not a valid prefix term syntax, recognized as simple term
CREATE PROCEDURE prefix_rewrite_prepare_p21 AS (SELECT sys.babelfish_fts_rewrite('"o@ne* "'));
GO
...
CREATE FULLTEXT INDEX ON fts_prefix_t(Content, ShortDescription, Notes)
KEY INDEX uid 
GO

CREATE VIEW fts_prefix_t_v1 AS (SELECT * FROM fts_prefix_t)
GO

CREATE VIEW fts_prefix_t_v2 AS (SELECT * FROM fts_prefix_t WHERE CONTAINS((Content, ShortDescription, Notes), '"comp*"'))
GO
...
CREATE PROCEDURE fts_multicol_prefix_t_p1 AS (SELECT * FROM fts_multicol_prefix_t WHERE CONTAINS((daily_updates,
                                                                                                  industry_news,
                                                                                                  local_events,
                                                                                                  tech_innovations,

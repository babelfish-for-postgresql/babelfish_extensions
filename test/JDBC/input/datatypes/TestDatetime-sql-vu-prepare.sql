...
AS(
    SELECT 
        ...
        CAST(CAST(2.5 as NUMERIC) as DATETIME)as re3,
        ...
        CAST(NULL as DATETIME)as res12
);
GO
...
AS(
    SELECT 
        ...
        CAST(CAST(2.5 as NUMERIC) as SMALLDATETIME)as re3,
        ...
        CAST(NULL as SMALLDATETIME)as res12
);
GO
...
AS(
    SELECT 
        ...
        CONVERT(DATETIME, CAST(2.5 as NUMERIC))as re3,
        ...
        CONVERT(DATETIME, NULL)as res12
);
GO
...
AS(
    SELECT 
        ...
        CONVERT(SMALLDATETIME, CAST(2.5 as NUMERIC)) as re3,
        ...
        CONVERT(SMALLDATETIME, NULL) as res12
);
GO
...
CREATE VIEW Datetime_view5 as (
    SELECT 
        ...
        DATEADD(minute, CAST(1 as NUMERIC), CAST(2.5 as NUMERIC)) as re2,
...
CREATE VIEW Datetime_view6 as (
    SELECT 
        ...
        DATEDIFF(minute, CAST(1 as NUMERIC), CAST(2.5 AS NUMERIC)) as re3,
...
CREATE PROCEDURE Datetime_proc3 (@a DATETIME, @b NUMERIC) AS
BEGIN
   DECLARE @c DATETIME = @b;
   SELECT (CASE WHEN @a = @c THEN 'pass' ELSE 'fail' END) as result;
END
...
CREATE PROCEDURE SMALLDatetime_proc3 (@a SMALLDATETIME, @b NUMERIC) AS
BEGIN
   DECLARE @c SMALLDATETIME = @b;
   SELECT (CASE WHEN @a = @c THEN 'pass' ELSE 'fail' END) as result;
END

...
SELECT ID, geometry::Parse(GeomColumn.STAsText()).STAsText() AS ParsedGeom FROM TestGeospatialParse_GeomTemp ORDER BY ID;
go
...
SELECT ID FROM TestGeospatialParse_GeomTemp WHERE geometry::Parse(@searchText).STEquals(GeomColumn) = 1 ORDER BY ID;
go
...
SELECT g1.ID, g2.ID FROM TestGeospatialParse_GeomTemp g1 
JOIN TestGeospatialParse_GeomTemp g2 ON geometry::Parse(@refText).STIntersects(g1.GeomColumn) = 1 
...
FROM TestGeospatialParse_GeomTemp ORDER BY ID;
go
...
WITH ParseCTE AS (
    ...
    FROM TestGeospatialParse_GeomTemp
)
...
SET @refText = 'POINT(3.0 4.0)';
SELECT ID, geometry::Parse(@refText).STDistance(GeomColumn) AS Distance 
FROM TestGeospatialMethods_DB.dbo.TestGeospatialParse_GeometryTable ORDER BY ID;
go

DECLARE @refText NVARCHAR(MAX);
SET @refText = 'POINT(3.0 4.0)';
SELECT ID, geography::Parse(@refText).STDistance(GeogColumn) AS Distance 
FROM TestGeospatialMethods_DB.dbo.TestGeospatialParse_GeographyTable ORDER BY ID;
go
...
FROM TestGeospatialParse_GeogTemp ORDER BY ID;
go

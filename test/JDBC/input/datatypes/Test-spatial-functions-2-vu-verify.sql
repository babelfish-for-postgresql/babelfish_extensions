...
FROM TestGeospatialMethods_STGeometryType_GEOM_Temp 
ORDER BY ID;
go
...
FROM TestGeospatialMethods_STGeometryType_GEOG_Temp 
ORDER BY ID;
go
...
SELECT * FROM TestGeospatialMethods_STGeometryType_GEOM_View_Temp ORDER BY ID;
go

SELECT * FROM TestGeospatialMethods_STGeometryType_GEOG_View_Temp ORDER BY ID;
go
...
SELECT ID, GeomColumn.STGeometryType() AS GeometryType FROM TestGeospatialMethods_STGeometryType_GEOM_Temp ORDER BY GeomColumn.STGeometryType();
go

SELECT ID,CASE GeomColumn.STGeometryType() WHEN 'Point' THEN 'Zero Dimensional' WHEN 'LineString' THEN 'One Dimensional' WHEN 'Polygon' THEN 'Two Dimensional'ELSE 'Unknown' END AS DimensionType FROM TestGeospatialMethods_STGeometryType_GEOM_Temp ORDER BY ID;
go

SELECT * FROM ( SELECT ID, GeomColumn.STGeometryType() AS GeomType FROM TestGeospatialMethods_STGeometryType_GEOM_Temp ) AS SubQuery WHERE GeomType = 'Point' ORDER BY ID;
go

SELECT g.ID AS GeomID, geo.ID AS GeogID, g.GeomColumn.STGeometryType() AS GeomType, geo.GeogColumn.STGeometryType() AS GeogType FROM TestGeospatialMethods_STGeometryType_GEOM_Temp g JOIN TestGeospatialMethods_STGeometryType_GEOG_Temp geo ON g.ID = geo.ID ORDER BY g.ID;
go

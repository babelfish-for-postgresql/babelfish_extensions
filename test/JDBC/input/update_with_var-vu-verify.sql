-- Test UPDATE
DECLARE @sum INT = 0;
UPDATE jira_babel_1290.test_int SET val = val, @sum = @sum + val;
SELECT @sum AS result; -- Expected: 60
GO

-- Test SELECT
DECLARE @sum2 INT = 0;
SELECT @sum2 = @sum2 + val FROM jira_babel_1290.test_int;
SELECT @sum2 AS result; -- Expected: 60
GO

-- Test UPDATE
DECLARE @concat VARCHAR(200) = '';
UPDATE jira_babel_1290.test_str SET name = name, @concat = @concat + name;
...
UPDATE jira_babel_1290.test_multi 
SET val = val, 
    @sum = @sum + val, 
    @concat = @concat + name + '|',
    @count = @count + 1;
...
DECLARE @sum INT = 0;
UPDATE jira_babel_1290.test_where SET val = val, @sum = @sum + val WHERE status = 'active';
SELECT @sum AS result; -- Expected: 40 (10+30)
GO

-- Test SELECT with WHERE
DECLARE @sum2 INT = 0;
SELECT @sum2 = @sum2 + val FROM jira_babel_1290.test_where WHERE status = 'inactive';
SELECT @sum2 AS result; -- Expected: 60 (20+40)
GO

-- Test with NULL values in numeric column
DECLARE @sum INT = 0;
UPDATE jira_babel_1290.test_null SET val = val, @sum = @sum + ISNULL(val, 0);
SELECT @sum AS result; -- Expected: 80
GO

-- Test with NULL values in string column
DECLARE @concat VARCHAR(200) = '';
UPDATE jira_babel_1290.test_null SET name = name, @concat = @concat + ISNULL(name, '');
SELECT @concat AS result; -- Expected: 'ABD'
GO

-- Test UPDATE on empty table
DECLARE @sum INT = 100;
UPDATE jira_babel_1290.test_empty SET val = val, @sum = @sum + val;
SELECT @sum AS result; -- Expected: 100 (unchanged)
GO

-- Test SELECT on empty table
DECLARE @sum2 INT = 100;
SELECT @sum2 = @sum2 + val FROM jira_babel_1290.test_empty;
SELECT @sum2 AS result; -- Expected: 100 (unchanged)
GO

-- Test UPDATE
DECLARE @sum INT = 10;
UPDATE jira_babel_1290.test_single SET val = val, @sum = @sum + val;
SELECT @sum AS result; -- Expected: 60
GO

-- Test with DECIMAL
DECLARE @dec_sum DECIMAL(10,2) = 0;
UPDATE jira_babel_1290.test_numeric SET int_val = int_val, @dec_sum = @dec_sum + decimal_val;
SELECT @dec_sum AS result; -- Expected: 31.00

-- Test with FLOAT
DECLARE @float_sum FLOAT = 0;
UPDATE jira_babel_1290.test_numeric SET int_val = int_val, @float_sum = @float_sum + float_val;
SELECT @float_sum AS result; -- Expected: 30.5
GO

-- Test with BIGINT
DECLARE @bigint_sum BIGINT = 0;
UPDATE jira_babel_1290.test_numeric SET int_val = int_val, @bigint_sum = @bigint_sum + bigint_val;
...
UPDATE jira_babel_1290.test_math SET val = val, @product = @product * val;
SELECT @product AS result; -- Expected: 24 (1*2*3*4)
GO

-- Test subtraction
DECLARE @diff INT = 100;
UPDATE jira_babel_1290.test_math SET val = val, @diff = @diff - val;
SELECT @diff AS result; -- Expected: 91 (100-2-3-4)
GO

-- Test mixed operations
DECLARE @mixed INT = 0;
UPDATE jira_babel_1290.test_math SET val = val, @mixed = @mixed + val * 2;
SELECT @mixed AS result; -- Expected: 18 (2*2 + 3*2 + 4*2)
GO

-- Test UPDATE where variable is used to update column
DECLARE @running INT = 0;
UPDATE jira_babel_1290.test_col_assign 
SET @running = @running + val, 
    running_total = @running;
SELECT * FROM jira_babel_1290.test_col_assign ORDER BY id;
...
FROM jira_babel_1290.test_main m
INNER JOIN jira_babel_1290.test_lookup l ON m.id = l.id;
...
WHILE @i <= 1000
BEGIN
    INSERT INTO jira_babel_1290.test_large VALUES (@i, @i);
    SET @i = @i + 1;
END
GO
...
-- Test UPDATE
DECLARE @sum INT = 0;
UPDATE jira_babel_1290.test_large SET val = val, @sum = @sum + val;
SELECT @sum AS result; -- Expected: 500500 (sum of 1 to 1000)
GO

-- Test SELECT
DECLARE @sum2 INT = 0;
SELECT @sum2 = @sum2 + val FROM jira_babel_1290.test_large;
SELECT @sum2 AS result; -- Expected: 500500
GO

-- Test complex string concatenation
DECLARE @result VARCHAR(200) = '';
UPDATE jira_babel_1290.test_str_complex 
SET prefix = prefix, 
...
UPDATE jira_babel_1290.test_case 
SET val = val,
...
BEGIN TRANSACTION;
    DECLARE @sum INT = 0;
    UPDATE jira_babel_1290.test_tran SET val = val, @sum = @sum + val;
...
DECLARE @sum INT = 0;
UPDATE jira_babel_1290.test_nested 
SET val = val, 
    @sum = @sum + (SELECT COUNT(*) FROM jira_babel_1290.test_other WHERE jira_babel_1290.test_other.id = jira_babel_1290.test_nested.id);
SELECT @sum AS result;
GO

-- Variable used in WHERE clause of same UPDATE
DECLARE @threshold INT = 10;
UPDATE jira_babel_1290.test_dynamic 
SET val = val, 
...
UPDATE jira_babel_1290.test_mixed_types 
SET val = val,
...
UPDATE jira_babel_1290.test_coalesce 
SET val = val,
...
UPDATE jira_babel_1290.test_running_max 
SET val = val,
...
USE test;
DECLARE @sum INT = 0;
UPDATE master.dbo.jira_babel_1290.test_cross_db 
SET val = val, 
    @sum = @sum + val;
SELECT @sum AS result;
GO
...
FROM jira_babel_1290.test_target target
INNER JOIN jira_babel_1290.test_source source ON target.id = source.id;

INSERT INTO jira_babel_1290.test_target (id, val)
SELECT id, val FROM jira_babel_1290.test_source 
WHERE id NOT IN (SELECT id FROM jira_babel_1290.test_target);
...
DECLARE @sum INT = 0;
UPDATE jira_babel_1290.test_udf 
SET val = val,
    @sum = @sum + jira_babel_1290.custom_function(val);
SELECT @sum AS result;
GO

-- Concurrent variable updates
DECLARE @counter1 INT = 0, @counter2 INT = 0;
UPDATE jira_babel_1290.test_concurrent 
SET val = val,
...
UPDATE jira_babel_1290.test_overflow 
SET val = val,
    @tiny = @tiny + val;
...
UPDATE jira_babel_1290.test_unicode 
SET name = name,
...
DECLARE @sum INT = 0;
UPDATE jira_babel_1290.test_computed 
...
WITH recursive_cte AS (
    ...
    FROM jira_babel_1290.test_recursive 
    WHERE parent_id IS NULL
    
    UNION ALL
    
    SELECT t.id, t.val, r.level + 1 
    FROM jira_babel_1290.test_recursive t
...
GO
SELECT * FROM jira_babel_1290.test_recursive ORDER BY id;
GO
...
----------------------------------------------------------------------------------------------------------------------
-- trigger scenarios
CREATE TABLE jira_babel_1290.simple_test (id INT, val INT);
INSERT INTO jira_babel_1290.simple_test VALUES (1, 10), (2, 20);
GO

CREATE TRIGGER trg_simple_test ON jira_babel_1290.simple_test
AFTER UPDATE AS
BEGIN
    INSERT INTO jira_babel_1290.simple_test VALUES (99, 99);
END;
GO

DECLARE @sum INT = 0;
UPDATE jira_babel_1290.simple_test SET val = val, @sum = @sum + val;
SELECT @sum AS result;
GO

DROP TRIGGER IF EXISTS trg_simple_test;
DROP TABLE jira_babel_1290.simple_test;
GO


CREATE TABLE jira_babel_1290.minimal_test (val INT);
INSERT INTO jira_babel_1290.minimal_test VALUES (5);
GO

CREATE TRIGGER trg_minimal ON jira_babel_1290.minimal_test
...
UPDATE jira_babel_1290.minimal_test SET val = val, @result = @result + val;
SELECT @result;
GO

DROP TRIGGER IF EXISTS trg_minimal;
DROP TABLE jira_babel_1290.minimal_test;
GO

-- UPDATE with OUTPUT clause and variables
DECLARE @sum INT = 0;
UPDATE jira_babel_1290.test_output 
SET val = val * 2,
    @sum = @sum + val

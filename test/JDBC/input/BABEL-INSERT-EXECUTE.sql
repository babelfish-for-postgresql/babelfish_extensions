...
-- nested insert execute
IF EXISTS (SELECT * FROM sys.procedures WHERE name = 'p8164a')
    DROP PROCEDURE p8164a
GO

IF EXISTS (SELECT * FROM sys.procedures WHERE name = 'p8164')
    DROP PROCEDURE p8164
GO

IF EXISTS (SELECT * FROM sys.tables WHERE name = 't8164')
    DROP TABLE t8164
GO

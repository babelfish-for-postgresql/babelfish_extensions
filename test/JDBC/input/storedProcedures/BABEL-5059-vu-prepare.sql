...
RETURNS TABLE
AS
    RETURN (SELECT STRING_AGG(col_nvarchar, '-') FROM babel_5059_t1 GROUP BY group_id ORDER BY group_id)
GO

CREATE FUNCTION babel_5059_itvf_func2()
RETURNS TABLE
AS
    RETURN (SELECT group_id,STRING_AGG(col_nvarchar, '-') FROM babel_5059_t1 GROUP BY group_id ORDER BY group_id)
GO

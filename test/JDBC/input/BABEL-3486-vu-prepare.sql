...
-- Convert numeric valid operations
-- Inconsistency NOTE: These tests do not perform the same way in babelfish as when they run on sql server
--             The precision parameter is accounted for but then each number is followed by trailing zeros

-- This test returns 5.33 in sql server and 5.33000000 in babelfish
...
-- This test returns -5.3 in sql server and -5.30000000 in babelfish
...
-- This test returns 5.0 in sql server and 5.00000000 in babelfish
...
-- This test returns 5 in sql server and 5.00000000
...
-- Inconsistency NOTE: This test does not perform the same way in babelfish as when they run on sql server
...
-- Convert float valid operations TESTS PASS
...
-- Convert float invalid operations TESTS PASS
...
-- This test returns 5.00 in sql server and null in babelfish
...
-- Convert char valid operations TESTS FAILED
--  ERROR NOTE: Using char in babelfish causes a tds protocol stream error
--              In sql server these tests work as expected
-- (SELECT TRY_CONVERT(char(30), CAST(5 AS MONEY), 8))
-- GO
-- (SELECT TRY_CONVERT(char(30), CAST('13:01:59' AS time), 8))
-- GO
-- (SELECT TRY_CONVERT(char(30), CAST(5 AS bigint), 8))
-- GO
-- (SELECT TRY_CONVERT(char(30), CAST(5 AS smallint), 8))
-- GO
-- (SELECT TRY_CONVERT(char(30), CAST(5 AS int), 8))
-- GO
-- (SELECT TRY_CONVERT(char(30), CAST(5 AS numeric), 8))
-- GO
-- (SELECT TRY_CONVERT(char(30), CAST(5.0 AS decimal), 8))
-- GO
-- (SELECT TRY_CONVERT(char(30), CAST(1 AS bit), 8))
-- GO
-- (SELECT TRY_CONVERT(char(30), CAST(1 AS smallmoney), 8))
-- GO
-- -- Convert char invalid operations
-- (SELECT TRY_CONVERT(char(1), CAST('2017-08-25' AS date), 8))
-- GO

-- Convert text valid operations TESTS PASS
...
-- Convert smalldatetime invalid TESTS PASS
...
-- Convert datetime valid TESTS PASS
...
-- Convert datetime invalid TESTS PASS
...
-- Convert datetime2 valid TESTS FAILED
-- Inconsistency NOTE: This test does not perform the same way in babelfish as when they run on sql server

-- This test returns 2017-08-25 13:01:59.00000 in sql server and returns null in babelfish
CREATE VIEW BABEL_3486_vu_prepare_v100 as (SELECT TRY_CONVERT(datetime2(5), '2017-08-25 13:01:59'));
GO
CREATE PROCEDURE BABEL_3486_vu_prepare_p100 as (SELECT TRY_CONVERT(datetime2(5), '2017-08-25 13:01:59'));
GO
CREATE FUNCTION BABEL_3486_vu_prepare_f100()
RETURNS datetime2 AS
BEGIN
RETURN (SELECT TRY_CONVERT(datetime2(5), '2017-08-25 13:01:59'));
END
GO

-- This test returns 2017-08-25 13:01:59.0 in sql server and returns null in babelfish
...
CREATE VIEW BABEL_3486_vu_prepare_v102 as (SELECT TRY_CONVERT(datetime2, '13:01:59'));
GO
CREATE PROCEDURE BABEL_3486_vu_prepare_p102 as (SELECT TRY_CONVERT(datetime2, '13:01:59'));
GO
...
-- Convert datetimeoffset invalid TESTS FAIL
...
-- Convert sql_variant valid TESTS PASS
...
-- Convert sql_variant invalid TESTS FAILED
-- Inconsistency Note: In sql server, conversion from image to sql_variant and conversion
--              from text to sql_variant are prohibited and throw an error
--              Babelfish does not perform the same way and instead supports the conversion

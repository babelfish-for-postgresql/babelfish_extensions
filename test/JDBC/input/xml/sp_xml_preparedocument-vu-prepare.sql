...
-- Test 7: View with aggregated XML data
CREATE VIEW XMLSummaryView_babel_1168 AS
SELECT 
    xml_type,
    COUNT(*) as xml_count,
    AVG(LEN(x.xml_content)) as avg_length,
    MAX(m.created_date) as latest_date
FROM XMLDataSource_babel_1168 x
INNER JOIN XMLMetadata_babel_1168 m ON x.id = m.xml_id
GROUP BY xml_type
GO

-- Test 8: Nested view with XML processing
...
-- Test 9: Parameterized view simulation with table-valued function
...
-- Test 10: View with XML error handling

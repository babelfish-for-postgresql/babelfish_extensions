...
FROM TestTypes;
GO
-- Test Case 8: Complex nested query with multiple operations
SELECT 
    outer_sum + (SELECT AVG(inner_product) FROM inner_query)
FROM (
    SELECT 
        SUM(CASE 
            WHEN IntCol > 150 THEN FloatCol 
            ELSE CAST(SmallIntCol AS float) 
        END) AS outer_sum
    FROM TestTypes
) AS outer_query
CROSS APPLY (
    SELECT 
        (UDTCol * NumericCol) + (IntCol / NULLIF(CAST(SmallIntCol AS float), 0)) AS inner_product
    FROM TestTypes
    WHERE IntCol = outer_query.outer_sum
) AS inner_query;
GO
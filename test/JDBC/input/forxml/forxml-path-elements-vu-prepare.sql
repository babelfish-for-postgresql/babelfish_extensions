...
CREATE TABLE forxml_path_elements_results (
...
CREATE PROCEDURE forxml_path_elements_p_insert AS
SELECT ID, Name FROM forxml_path_elements_t1 WHERE ID <= 2 FOR XML PATH('Employee'), ELEMENTS;
GO

CREATE PROCEDURE forxml_path_elements_p_insert_xsinil AS
SELECT ID, Name FROM forxml_path_elements_t1 WHERE ID <= 2 FOR XML PATH('Employee'), ELEMENTS XSINIL;
GO

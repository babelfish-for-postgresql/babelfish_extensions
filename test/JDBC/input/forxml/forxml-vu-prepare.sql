create table forxml_vu_t1 (id int, a varchar(10));
create table forxml_vu_t2 (id int, a varchar(10));
insert into forxml_vu_t1 values (1, 't1_a1');
insert into forxml_vu_t1 values (2, 't1_a2');
insert into forxml_vu_t1 values (3, NULL);
insert into forxml_vu_t2 values (1, 't2_a1');
insert into forxml_vu_t2 values (2, 't2_a2');
insert into forxml_vu_t2 values (3, NULL);
go
...
select * from forxml_vu_t1 for xml raw('myrow');
go

select * from forxml_vu_t1 for xml path;
go

select * from forxml_vu_t1 for xml path('myrow');
go

select * from forxml_vu_t1 for xml explicit;
go

select * from forxml_vu_t1, forxml_vu_t2 where forxml_vu_t1.id = forxml_vu_t2.id for xml path('myrow');
go

select * from forxml_vu_t1, forxml_vu_t2 where forxml_vu_t1.id = forxml_vu_t2.id for xml path('myrow'), type;
go

select * from forxml_vu_t1, forxml_vu_t2 where forxml_vu_t1.id = forxml_vu_t2.id for xml path('myrow'), type, root('myroot');
go

select * from forxml_vu_t1, forxml_vu_t2 where forxml_vu_t1.id = forxml_vu_t2.id for xml path('myrow'), type, root('myroot'), BINARY BASE64;
go

-- Test result format when all values are NULL
select a from forxml_vu_t1 where a is NULL for xml raw;
go

select a from forxml_vu_t1 where a is NULL for xml path;
go

-- Test result format when one value is NULL
select id, a from forxml_vu_t1 where a is NULL for xml raw;
go

select id, a from forxml_vu_t1 where a is NULL for xml path;
go

-- Test for xml with order by clause
select * from forxml_vu_t1 order by id for xml raw ('t1');
go

-- Test for xml with group by
select count(*) as cnt, a from forxml_vu_t1 group by a,id order by id;
go
select count(*) as cnt, a from forxml_vu_t1 group by a,id order by id for xml path;
go

-- Test for xml in subquery, The subquery is supposed to return only one XML value
select id, (select a from forxml_vu_t2 for xml path) as col from forxml_vu_t1;
go

-- Test 2 levels of for xml nesting, with TYPE option
select id, (select a from forxml_vu_t2 for xml path, type) as col from forxml_vu_t1 for xml path, type;
go

-- Test 2 levels of for xml nesting, without TYPE option, expect special character escaping
select id, (select a from forxml_vu_t2 for xml path) as col from forxml_vu_t1 for xml path;
go

-- Test 3 levels of for xml nesting with TYPE option
select id, (select id, (select a from forxml_vu_t2 for xml path, type) as col1 from forxml_vu_t1 for xml path, type) as col2 from forxml_vu_t1 for xml path, type;
go
...
create procedure forxml_vu_p_employee_select as
begin
...
create procedure forxml_vu_p_employee_select2 @minsal MONEY, @maxsal MONEY as
begin
...
select a from forxml_vu_t1 UNION ALL select a from forxml_vu_t2 for xml raw ('myrow');
go

-- Test invalid syntax when FOR XML is on both sides of UNION ALL, this is SQL Server behavior
select a from forxml_vu_t1 for xml raw ('t1') UNION ALL select a from forxml_vu_t2 for xml raw ('t2');
go

-- For xml can access CTE from same query block
create view forxml_vu_v_cte1 as
with cte as (select a from forxml_vu_t1)
...
-- function
create procedure forxml_vu_p_datalength @pid int as
declare @a int, @b smallint;
set @a = 1;
set @b = 1;
select a, datalength(@a), datalength(@b) from forxml_vu_t1 where id = @pid for xml raw;
go
